...
public class DbSession:IDbSession {
      ...
      private readonly IDbSessionDAL sessionDAL;
        ...
        private void CreateSessionCookie(Guid sessionid)
        {
            ...
            options.Secure = true;
            httpContextAccessor?.HttpContext?.Response.Cookies.Delete(AuthConstans.SessionCookieName);
            httpContextAccessor?.HttpContext?.Response.Cookies.Append(AuthConstans.SessionCookieName, sessionid.ToString(), options);
        }
        ...
        public async Task<SessionModel> GetSession()
        {
            ...
            Guid sessionId;
            var cookie = httpContextAccessor?.HttpContext?.Request?.Cookies.FirstOrDefault(m => m.Key == AuthConstans.SessionCookieName);
